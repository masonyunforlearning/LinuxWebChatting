                        ...
                        if (!strcmp(ptr, "ROOM"))
                        {
                            ...
                            if (!strcmp(ptr, "ENTE"))
                            {
                                ...
                                if(!strcmp(ptr2,Server_room[atoi(ptr)].password)) //password correct
                                {
                                    ...
                                    printf("%d ?\n", Server_room[atoi(ptr)].cur_member);
                                    for (k = 0; k < Server_room[atoi(ptr)].max_member; k++)
                                    {
                                    ...
                                    if(k == Server_room[atoi(ptr)].max_member)
                                    {
                                        sprintf(buf, "_##_SERV_##_ENTE_##_FAIL_##_");
                                        write(i, buf, BUF_SIZE);
                                    }else {
                                        Room_member_info[i].curr_roomNumber = atoi(ptr);
                                        printf("%s : %d\n", Room_member_info[i].nickname, atoi(ptr));
                                        sprintf(buf, "_##_SERV_##_ENTE_##_SUCC_##_");
                                        write(i, buf, BUF_SIZE);
                                    }
                                }
                                else //password fault
...
void init_room(room* server_rooms, socketinfo* infos, char* room_name, int id, char* password, char* max_member)
{
    ...
    if(i == 10) {
        sprintf(room_buffer, "_##_SERV_##_MAKE_##_FAIL_##_");
        write(id, room_buffer, BUF_SIZE);       
    }
}