...
int chatting(int sock_id)
{
    ...
    while (1)
    {
        ...
        if (message[0] == '/')//명령어 파트(dm포함)
        {
            ...
            {
                endwin();
                write(sock_id, "_##_ROOM_##_QUIT_##_",BUF_SIZE);
                exit(1);
                // return 1;
            }
            ...
            else
            {
                ...
                else if (!strcmp(token, "/mute"))
                {
                    token = strtok(NULL, " ");
                    if (token == NULL || token[0] == '\0')
                    {
                        wprintw(log_win, "/mute 형식에 맞지 않습니다. 다시 시도해 주세요 (/mute <닉네임>)\n\n");
                    }
                    else
                    {
                        strcpy(ban[ban_max], token);
                        wprintw(log_win, "이제부터 %s의 메세지는 차단됩니다.\n\n", token);
                        ban_max++;
                    }
                ...
                else if (!strcmp(token, "/mkroom"))
                {
                    ...
                    if (intheroom)
                    {
                        wprintw(log_win, "/mkroom 형식에 맞지 않습니다. 다시 시도해 주세요 (/mkroom <방이름> <비밀번호> <최대인원>)\n\n");
                    }
                    ...
                    {
                        sprintf(buf, "%s%s%s%s", ROOM, MAKE, token, protocol);
                        token = strtok(NULL, " ");
                        if (token == NULL || token[0] == '\0')
                        {
                            wprintw(log_win, "/mkroom 형식에 맞지 않습니다. 다시 시도해 주세요 (/mkroom <방이름> <비밀번호> <최대인원>)\n\n");
                        ...
                        else
                        {
                            ...
                            if (token == NULL || token[0] == '\0')
                            {
                                sprintf(buf, "%s%s%s%s", buf, protocol, token2, protocol);
                                write(sock_id, buf, strlen(buf));
                            }
                            else
                            {
                                sprintf(buf, "%s%s%s%s%s", buf,token2 ,protocol, token, protocol);
                                write(sock_id, buf, strlen(buf));
                            }
                        }
                    }
                ...
                else if (!strcmp(token, "/enter"))
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (token == NULL || token[0] == '\0')
                            {
                                wprintw(log_win, "/enter 형식에 맞지 않습니다. 다시 시도해주세요 (/enter <방번호_필수> <비밀번호>)\n\n");
                            }
                            else
                            {
                                sprintf(buf, "%s%s%s%s", ROOM, ENTER, token, protocol);
                                token = strtok(NULL, " ");
                                if (token == NULL)
                                {
                                    sprintf(buf, "%s%s", buf,protocol);
                                }
                                else
                                {
                                    sprintf(buf, "%s%s%s", buf, token, protocol);
                                }
                                sprintf(buf, "%s%s%s", buf,token, protocol);
                                write(sock_id, buf, strlen(buf));
                                // intheroom++;
...
int received(int sock_id)
{
    ...
    {   
        ...
        if(!strcmp(ptr, "ENTE"))
        {
            ...
            if(!strcmp(ptr, "FAIL"))
            {
                wprintw(log_win, "Fail to enter room\n");
            }
        }
        if(!strcmp(ptr, "MAKE"))
        {
            ...
            if(!strcmp(ptr, "FAIL"))
            {
                wprintw(log_win, "Fail to make room\n");
            }
        }
    }else {
        int i;
        wprintw(log_win, "%s", message2);
    }
...
void seecommand() {
    ...
    wprintw(log_win, "/mkroom <방이름> <비밀번호> <방인원>: 입력한 이름으로 채팅방을 생성, *비밀번호 필수*\n");
    wprintw(log_win, "/enter <방번호> <비밀번호> : 입력한 번호의 채팅방으로 입장\n");
    wprintw(log_win, "/exit : 채팅방 나가기\n");
    wprintw(log_win, "/info : 현재 채팅방의 정보 보기\n");
    wprintw(log_win, "/dm <Target> <message> : 입력한 대상에게 귓속말을 보냄.\n");
    // wprintw(log_win, "/mute <Target> : 앞으로 대상의 메세지 차단.\n\n");
}
